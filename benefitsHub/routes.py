...
def upload_file():
    ...
    return render_template('create_benefit.html')
...
def user_benefits(username):
    page = request.args.get('page', 1, type=int)
    user_benefit = Benefit.query.filter_by(user=username).first_or_404()
    benefits = Benefit.query.filter_by(user=user_benefit)\
